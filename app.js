...
function pretty(key) {
  return ({
    ...
    last5Avg: 'L5', h2hAvg: 'H2H', projection: 'Proj', edge: 'Edge', pick: 'Side'
  })[key] || key;
}
...
function applyPitcherFilters(rows) {
  ...
  if (pitcherFilter === 'overs') filtered = filtered.filter(r => String(r.pick || '').toUpperCase() === 'OVER');
  if (pitcherFilter === 'unders') filtered = filtered.filter(r => String(r.pick || '').toUpperCase() === 'UNDER');
...
function renderCell(row, key) {
  ...
  if (key === 'pick') {
    const value = hasLine(row) ? (row[key] || 'NO LINE') : 'NO LINE';
    const css = String(value).toLowerCase().replace(/\s+/g, '-');
    return `<span class="pick-pill ${css}">${formatValue(value)}</span>`;
  }
